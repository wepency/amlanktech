...
(function ($) {
    ...
    $window.on('load', function () {
        ...
        if($(topNav).length) {
            var topOfNav = topNav.offsetTop;
				...
				$('body').on('click', '.sidebar-widget', function (e){
                    if($(e.target).is('.sidebar-title') || $(e.target).is('.sidebar-title *') || $(e.target).is('.la'))
                        $(this).toggleClass('active');
				});
				...
				// 	$('.mobile-search-box').removeClass('show').hide()
        ...
        // }
				...
				// $("#scroll-to-availability").click(function(e) {
				// 	e.preventDefault();
				// 	console.log('clicked');
				//
	...
	$('body').click(function(e) {
		if (!$(e.target).parents('.navbar-toggler').length &&  !$(e.target).hasClass('navbar-toggler')) {
			...
			$('.navbar-toggler').removeClass('nav-open')
		}
	});
