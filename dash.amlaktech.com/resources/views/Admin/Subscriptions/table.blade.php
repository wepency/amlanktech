                ...
                <table id="data-table"
                    ...
                    @foreach($subscriptions as $subscription)
                        <tr id="{{$subscription?->id}}">
                            ...
                            <td>{{$subscription?->subscriptionType->name_ar}}</td>
                            ...
                            <td>
                                @if($subscription?->is_paid == 0)
                                    <i class="fa fa-times"></i> متعثرة
                                @else
                                    <i class="fa fa-check"></i> تم الاشتراك
                            @endif
                            <td>

                                <!-- Edit -->
                                <button type="button" class="btn btn-primary" data-toggle="tooltip" title="تعديل"
                                        data-bs-toggle="modal"
                                        data-bs-target="#edit-subscription-{{$subscription->id}}"><i
                                        class="far fa-edit"></i></button>

                                <!-- Delete -->
                                <form method="post"
                                      action="{{route('dashboard.subscriptions.destroy', [ 'subscription' => $subscription->id])}}"
                                      style="display:inline-block;margin:0">
                                    @csrf
                                    @method('delete')

                                    <button type="submit" class="btn btn-danger" data-toggle="tooltip" title="الحذف"><i
