...
class PermitsController extends Controller
{
    ...
    public function index(Request $request, Permit $permits)
    {
        ...
        if ($request->ajax()) {
            return $this->permitsDataTable($request, $permits, 'index');
        }
    ...
    {
        $permits = $permits->with('association', 'member', 'visitors')->whereNull('status');
        ...
        if ($request->ajax()) {
            return $this->permitsDataTable($request, $permits, 'requests');
        }

        return view('Admin.Permits.Requests', [
            'page_title' => 'طلبات التصريح',
            'permits' => $permits
        ]);
    }

    private function permitsDataTable($request, $permits, $type)
    {
        return DataTables::eloquent($permits)
            ...
            ->addColumn('action', function ($row) use ($type) {
                ...
                if ($type == 'requests') {
                    return $this->getPermitRequestButtons($row);
                } else {
                    return $this->getPermitsButtons($row);
                }
            })
...
//                    }
    ...
    private function getPermitRequestButtons($row)
    {
        $out = '<div class="table-buttons">';

        $out .= '<form method="post" action="' . dashboard_route('permits.requests.trigger', [$row->id, 'accept']) . '" style="display:inline-block;margin:0">';
        $out .= csrf_field();
        $out .= method_field('put');
        $out .= '<button type="submit" class="btn btn-success" data-toggle="tooltip"
                                                    title="الحذف"><i class="fas fa-check"></i></button>';
        $out .= '</form>';

        $out .= '&nbsp';

        $out .= '<form method="post" action="' . dashboard_route('permits.requests.trigger', [$row->id, 'refuse']) . '" style="display:inline-block;margin:0">';
        $out .= csrf_field();
        $out .= method_field('put');
        $out .= '<button type="submit" class="btn btn-danger" data-toggle="tooltip"
                                                    title="الحذف"><i class="fas fa-times"></i></button>';
        $out .= '</form>';

        return $out;
    }

    private function getPermitsButtons($row)
    {
        $out = '<div class="table-buttons">';
    ...
    public function triggerRequest(Request $request, Permit $permit, $type)
    {
        if ($type == 'accept') {
            $permit->update([
                'status' => 1
            ...
            return $this->redirectBack()->withSuccess('تم قبول التصريح بنجاح.');
        }

        $permit->delete();
        return $this->redirectBack()->withSuccess('تم رفض التصريح.');
    }
